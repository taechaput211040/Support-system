<template>
  <div class="container">
    <h2 class="text-center mt-2 mb-4">จัดการหน้า Member</h2>
    <div class="row pa-3 mt-3">
      ...
      <v-alert dense text type="success" v-show="!hashStatic"
        >เลือกเว็ปที่ต้องการด้านบนแล้วกดค้นหาเพื่อตั้งค่าหน้า Member</v-alert
      >
      <iframe
        v-show="hashStatic"
        id="myIframe"
...
export default {
  ...
  methods: {
    ...
    async getIframeRender(hash) {
      ...
      let linked = `https://admin-salepage.zamba777.com/#/member?hash=${hash}`;
      console.log(linked);
